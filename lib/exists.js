 ...
 * @param callback
 */
function exists(module, prefix, hashcode, log, callback) {
      ...
      if (err) {
        ...
        try {
          ...
          if (hash === hashcode) {
            ...
            log.debug('NPMResolver', module + ' local hashcode differs from model');
            log.debug('NPMResolver', 'local:'+hash+' != model:'+hashcode);
            callback(null, false);
          }
        } catch (error) {
