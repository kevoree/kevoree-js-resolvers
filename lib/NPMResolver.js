  ...
  exists(options.name, options.path, options.hashcode, function (err, doesExist, hashError) {
    if (err) {
      ...
      if (doesExist && hashError) {
        this.log.debug(this.toString(), options.name + ' local install differs from registry');
          ...
          if (err) {
            ...
            exists(options.name, options.path, options.hashcode, function (err, doesExist, hashError) {
              if (err) {
                ...
                if (doesExist && !hashError) {
                  this.log.info(this.toString(), options.name + ' installed');
                  this.loadClass(pkgPath, callback);
                } else {
                  this.log.error(this.toString(), options.name + ' integrity check failed');
                  this.log.error(this.toString(), 'This probably means that you have (re-)published your DeployUnit to the Kevoree registry but it now differs with the one on the npm registry');
