...
var NPMResolver = Resolver.extend({
    ...
    uninstall: function (deployUnit, callback) {
        execNpm([ 'uninstall', deployUnit.name, '--prefix=' + this.modulesPath ], { stdio: 'inherit' }, function (err) {
            if (err) {
        ...
        exists(options.name, options.path, function (err, exists) {
            if (err) {
                ...
                if (exists) {
                    ...
                    execNpm([ 'install', options.name, '--prefix=' + options.path ], { stdio: 'inherit' }, function (err) {
                        if (err) {
